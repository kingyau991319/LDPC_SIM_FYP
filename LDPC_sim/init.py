...
class Init:
    ...
    def __init__(self,noise_set,mult_num,LDPC_type,iteration,clip_num,f_name=""):
    ...
    def signal_tranmission_repeat(self,message_sending_time,print_flag):
        ...
        for k in range(message_sending_time):
            # codeword_hamming_dist,no_bit_error_flag,no_bit_type_flag,codeword_hard_hamming_dist = self.message_sending_simulation(print_flag)
            codeword_hamming_dist,no_bit_error_flag,no_bit_type_flag,codeword_hard_hamming_dist = self.message_sending_simulation2(print_flag)
        ...
        prob_BER = average_probaility_error_summation / (message_sending_time * self.ylen)
    ...
    def message_sending_simulation2(self,print_flag):

        # message generator
        xlen, ylen = self.xlen, self.ylen
        msg = np.random.randint(2,size=(ylen,ylen))
        generator_matrix,parity_check_matrix = self.generator_matrix,self.parity_check_matrix

        msg_copy = msg.copy() # to backup and compare the result

        # mult it and transpose and matmul one more time
        msg = np.matmul(msg,generator_matrix)
    ...
    def message_sending_simulation(self,print_flag):
...
def LDPCCode_running(len_mult_num,noise_set,message_sending_time,iteration,name,LDPC_type,clip_num,filename):

    time_start = time.time()
    wr = 10
    wc = 5

    init = Init(noise_set,len_mult_num,LDPC_type,iteration,clip_num,filename)
...
def runOnlyOneTimeLDPC():
    ...
    init = Init(noise_set,len_mult_num,LDPC_type,iteration,7,"")
...
if __name__ == "__main__":
    ...
    name = "Product_code_96.33.964"
    filename = "test_code_96.33.964"

    noise_set,message_sending_time = 0.75,100
    LDPCCode_running(len_mult_num,noise_set,message_sending_time,iteration,name,LDPC_type,clip_num,filename)

    noise_set,message_sending_time = 0.7,100
    LDPCCode_running(len_mult_num,noise_set,message_sending_time,iteration,name,LDPC_type,clip_num,filename)