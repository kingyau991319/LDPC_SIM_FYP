...
'''
    ...
    this part is for decoding for different methods.
    btw now noly implement with SPA algoirthm, it is a type of belief proggargation
'''
# @jitclass([('parity_check_matrix',types.double[:,:]),
            # ('p_col',types.int64),
            # ('p_len',types.int64),
            # ('t',types.int64)]) 
class decoding_algorithm(object):
    ...
    '''
        sumProductAlgorithm function is using Sum-Product Algorithm(SPA) for decoding a LDPC code
        ...
        output:
            the final message of the result
    '''
    ...
    def sumProductAlgorithmWithIterationForPC(self,input_matrix,iteration):
        ...
        for iteration_time in range(iteration):
            ...
            mult_summation = np.where(parity_matrix_copy_add_part==0,1.0,np.tanh(parity_matrix_copy_add_part/2))
    ...
    def BDD(self,hamming_distance,t):
        return 0
...
if __name__ == "__main__":
    ...
    decoding = decoding_algorithm(parity_matrix,2,5)
